...
class AudioRecorder extends HTMLElement {
    ...
    try {
      ...
      this.mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          // Convert to MP3
          this.mp3Blob = await this.convertToMp3(this.blob);
  ...
  async submitRecording() {
    ...
    formData.append('mp3file', this.mp3Blob, 'recording.mp3'); // Changed to MP3
  ...
  async encodeMp3(audioBuffer) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof lamejs === 'undefined') {
        ...
        script.onload = () => {
          this.performMp3Encoding(audioBuffer, resolve, reject);
        };
        script.onerror = () => {
  ...
  performMp3Encoding(audioBuffer, resolve, reject) {
    try {
      const numChannels = audioBuffer.numberOfChannels;
      const sampleRate = audioBuffer.sampleRate;
      ...
      let rightData = numChannels > 1 ? audioBuffer.getChannelData(1) : null;
...
customElements.define('audio-recorder5555', AudioRecorder);
