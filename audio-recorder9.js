class AudioRecorder extends HTMLElement {
  ...
  try {
    ...
    // Get new upload URL from Wix
    const uploadUrlResponse = await fetch('https://www.thementorprogram.xyz/_functions/get_uploadUrl');
    const uploadUrlData = await uploadUrlResponse.json();
    const wixUploadUrl = uploadUrlData.url;
    ...
    // Upload file to Wix using the new upload URL
    const wixResponse = await fetch(wixUploadUrl, {
      method: 'POST',
      body: formData
    });
    ...
    // Continue to send the file to Zapier
    ...
    if (zapierResponse.ok) {
      ...
      throw new Error(`Server responded with status ${zapierResponse.status}`);
    }
  } catch (error) {
...
customElements.define('audio-recorder9', AudioRecorder);
